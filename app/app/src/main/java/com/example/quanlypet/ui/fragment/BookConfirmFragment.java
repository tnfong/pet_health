...
public class BookConfirmFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public BookConfirmFragment(MainActivity activity) {
    ...
    private void loadDataBook(){
        ...
        LiveData<CommonResponse<List<Book>>> lvData = this.activity.getMainViewModel().bookList(uid, 4);
        lvData.observe(getActivity(), data -> {
            if(data.getStatus()){
                if (UserUtils.isAdmin() || UserUtils.isDoctor()) {
