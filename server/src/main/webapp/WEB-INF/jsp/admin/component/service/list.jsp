    ...
    <div id="app" class="app app-header-fixed app-sidebar-fixed">
		...
		<div id="content" class="app-content">
			...
			<h1 class="page-header d-flex align-items-center">
			  Quản lý dịch vụ <small class="ms-2"><a href="/service/add" class="btn btn-success btn-sm"><i class="fa fa-plus-circle me-2"></i> Thêm mới</a></small>
			</h1>
							...
							<c:if test="${data.getContent().size() > 0}">
								<c:forEach items="${data.getContent()}" var="item" varStatus="loop">
                                    <tr>
                                        ...
                                        <td><a href="/service/update/${item.id}"><i class="fa fa-cog"></i></a></td>
                                    </tr>
								</c:forEach>
							</c:if>
							<c:if test="${data.getContent().size() == 0}">
